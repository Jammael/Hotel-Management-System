<?php
// require user to be logged in (temp_session holds active user)
$conn = new mysqli("localhost","root","", "iwp");
if($conn->connect_error){
	// In case DB is down, show a simple message (avoid headers already sent)
	die("Connection failed: " . $conn->connect_error);
}
$sql = "SELECT * from temp_session";
$result = mysqli_query($conn, $sql);
$row = mysqli_fetch_row($result);
// require both a temp_session row and a matching hotel_auth cookie
$logged = false;
if($row && isset($_COOKIE['hotel_auth']) && $_COOKIE['hotel_auth'] == $row[0]){
	$logged = true;
}

if(!$logged){
	$current = $_SERVER['REQUEST_URI'];
	header("Location: user_login.php?next=" . urlencode($current));
	exit();
}
?>
<!DOCTYPE html>
...
<body>
	<?php // session checked at top of file; $conn and $row are available ?>
